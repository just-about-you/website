...
`62-medications.webp` **was** on the landing page twice and MM260 had
deleted that screen outright. WW140 replaced both with
`83-medications-full`, the screen that took over. That green was the expensive
...
on Clover. That is exactly what this section said would clear them, and it
cleared them with no change in this repo, as predicted. WW120 then re-synced
the site's copies: 44 were stale against the new sources, 7 features had never
had a picture at all, and 1 was an orphan of a screen TT110 deleted.

**The twelfth was `62-medications`**, and it was the one no capture run could
fix: it photographed the manage-medications list MM260 deleted. This section
said clearing it meant a different picture or none, that it was a content
decision and the operator's, and that it was flagged rather than taken. That
held. WW140 put it to the operator, who chose `83-medications-full` — the
screen that took over, already carried on `features/medications.html`.

**The operator ruling of 2026-09-01, kept verbatim:**

> Accept the red build until EE192. The pictures stay up. The site stays
> un-deployable until the captures are re-shot.

Recorded at that length because the operator named the risk in the act of
taking it: *a standing red build invites someone to "fix" it by softening the
guard.* Worth keeping now that it is green, because that is the sentence a
future red should be read against.

- **What did not clear it.** Softening, skipping, allow-listing or
  grace-periodding `check.py`. PP440 made the build *redder* by adding the
  retired-capture guard, on the reasoning that the decision was to accept a
  red build and not to cap how red it is. **WW100 is not an exception**: it
  made the stale reader tell three states apart instead of two, removing a
  false alarm and loosening no assertion. An absent declaration still fails,
  and `python scripts/check.py --self-test` proves it still does.
- **What the red cost while it stood.** Nothing published, which was the
  point — but also that a *new* failure would have arrived into a log already
  full of expected ones. Twelve known failures hide a thirteenth. That is the
  argument for clearing a deliberate red promptly rather than living in it.
